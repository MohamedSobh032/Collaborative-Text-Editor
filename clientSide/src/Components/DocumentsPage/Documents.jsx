    ...
    const fetchDocuments = async () => {
      try {
        const response = await fetch(
          `http://localhost:8080/api/mixed/${props.user.username}/GetDocuments`
        );
        if (!response.ok) {
    ...
    <div>
      ...
      <div className="section">
        ...
        <div className="document-list">
          {documents.filter((document) => document.accessType === "OWNER")
            .length === 0 ? (
            <h2 className="no-documents">No Documents Available</h2>
          ) : (
            documents
              .filter((document) => document.accessType === "OWNER")
              .map((document) => (
